...
async fn get_one_json(project_key: &str, config: &Config, start: i64, max_result_per_query: i32) -> Result<JsonValue, String> {
...
async fn get_issues_from_json_that_need_updating<'a>(json_data: &'a Value,
                                                     db_conn: &Pool<Sqlite>)
  -> Result<(Vec<issue_data>, FoundIssueUpToDate), String>
{
  ...
  let Some(json_data) = json_data else {
    let err_msg = "No issue was found in the json passed to get_issues_from_json_that_need_updating";
    eprintln!("{err_msg}");
  ...
  let query_str =
    "SELECT field_value as timestamp
     FROM IssueField
    ...
    for issue in issues_to_check {
      ...
      match row {
        Ok(Some(data)) => {
          ...
          eprintln!("Comparing\n{a}\n{b}\n\n", a = timestamp_to_use, b = issue.last_updated);
          if timestamp_to_use == issue.last_updated {
            return Ok((issues_to_update, FoundIssueUpToDate::ONE_ISSUE_IS_UP_TO_DATE));
          }
...
pub(crate) async fn get_issues_that_need_updating(project_key: &str,
                                                  config: &Config,
                                                  db_conn: &Pool<Sqlite>)
  -> Result<Vec<issue_data>, String> {
  eprintln!("Querying issues/tasks for project {project_key} in search of tickets that need updating");
  ...
  let max_result_per_query = first_json.as_object()
    .and_then(|x| x.get("maxResults"))
    .and_then(|x| x.as_i64())
    .unwrap_or_else(|| {
      eprintln!("Couldn't retrieve the number of max results from the jira server. Using 100");
      100
    });
  ...
  let first_issues_to_update = get_issues_from_json_that_need_updating(&first_json, db_conn).await;
  ...
  for i in 0..(total / max_result_per_query) {
    ...
    match next_json {
      Ok(next_json) => {
        let new_issues_to_update = get_issues_from_json_that_need_updating(&next_json,
                                                                           db_conn).await;
